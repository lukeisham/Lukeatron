...
- **Medium-term** (`Memory/Medium-Term/`) — pruned on demand via `!PruneMemory`. `Projects/` (colour board `_tracking.yaml`; owned by `!ProjectSweep`; board on `localhost:8789` via the **Dashboard**, see *Browser tools* below), `Contacts/` (temp contacts, always non-listed), `temp-skills/`, `file-locations.md`.
...
**Browser tools** — read-only-over-Long-Term local viewers, each kept live on its port by a `SessionStart` hook in `.Claude/settings.json`: the **Dashboard** (a.k.a. **Project Dashboard** — Luke's nomenclature, 2026-09-14; `System/Apps/ProjectKanban/_template/`, `:8789`) over `Projects/`, and **LukeatronWiki** (`System/Apps/LukeatronWiki/`, `:8787`) over `Memory/Long-Term/` (see *Memory Structure*). Both: `python3 server.py` or a double-click `.command` launcher; neither ever writes Long-Term content of its own accord — LukeatronWiki's own generation gate and quick-capture are the sole, narrowly-scoped exceptions (see `Memory/Long-Term/Lukeatron/memory-structure.md`).

> **Naming note — Dashboard vs ProjectKanban.** The viewer app itself is called the **Dashboard** / **Project Dashboard** in conversation, chat, and anything user-facing. `ProjectKanban` is kept as the app's internal folder name (`System/Apps/ProjectKanban/`) and, deliberately, as the name of the tracked Medium-Term project (LU-02) that organises work to build or fix that viewer — Luke's explicit call, so that "fix the Dashboard" (the app) and "advance ProjectKanban" (the project improving it) never collide into a self-referential loop. Skills and triggers should read "dashboard" / "project dashboard" as this app.

The Dashboard app replaced **Project Dashboard** (`System/Apps/ProjectDashboard/`), retired 2026-09-12 and archived to `Archive/ProjectDashboard-app-2026-09-12/`; that tool's own predecessor (`System/Tools/project-dashboard/`) was retired 2026-09-09 and archived to `Archive/ProjectDashboard-tool-2026-09/`. LukeatronWiki replaced the **LukeatronWiki viewer** (`System/Tools/lukeatronwiki-viewer/`), retired 2026-09-12 and archived to `Archive/lukeatronwiki-viewer-tool-2026-09/`. The Skillbank `!Dashboard` skill (an unrelated ephemeral `show_widget` snapshot, unconnected to this app) was itself retired 2026-09-14 for colliding on the same name — archived to `Archive/Dashboard-skill-2026-09-14/`.
