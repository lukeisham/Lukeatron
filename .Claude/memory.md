...
- **Chat and internal reports: five STE sentence rules.** A light subset of `!SimpleEnglish` (Skillbank, pragmatic mode) applied on top of `!PlainEnglish`'s four-part shape — the full skill is NOT loaded for chat. Rules: (1) sentences about 20–25 words at most; (2) one name per thing — never "config" here and "settings" there; (3) condition before command ("If X, do Y"); (4) delete filler ("simply", "leverage", "it is worth noting that"); (5) no contractions, and keep "that". Scope is Claude's own internal writing only; outgoing content follows `!Tone`. Verbatim quotes, code and evidence stay untouched. Full STE rules (banned modals, no `-ing` verbs, no semicolons) are deliberately not adopted.
