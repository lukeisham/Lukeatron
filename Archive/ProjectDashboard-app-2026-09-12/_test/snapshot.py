...
<head>
  ...
  <title>Project Dashboard — Snapshot — TEST</title>
  <style>
{css}
  </style>
</head>
...
def build_snapshot_html(*, today: date, build_time: datetime, board: model.Board | None = None) -> str:
    ...
    # TEST-BUILD LABEL — the leading "🧪 TEST DATA —" is the one line this
    # module's stamp carries that _template/snapshot.py's does not
    # (!AppDevelopment phase4-refactor.md STEP 5 divergence allowlist: "the
    # TEST banner/title/launcher markers"). Same #snapshot-stamp element,
    # same tokens-only CSS below — the label rides the banner that already
    # exists (FR-3) rather than adding a second one.
    stamp_text = (
        f"🧪 TEST DATA — SNAPSHOT — as at {build_time.strftime('%Y-%m-%d %H:%M')} — "
