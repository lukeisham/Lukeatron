...
def compose_brief(board: model.Board, *, today: date, stamp: datetime) -> str:
    ...
    sections: list[list[str]] = [
        # TEST-BUILD LABEL — the "🧪 TEST DATA" line is the one line this
        # module's output carries that _template/brief.py's does not
        # (!AppDevelopment phase4-refactor.md STEP 5 divergence allowlist:
        # "the TEST banner/title/launcher markers"). Plain prose, so it is
        # never mistaken for a section heading and never touches
        # assert_no_retired_terms (it names no retired term).
        ["# Board Brief", "", "🧪 **TEST DATA — a fabricated board, not Luke's real projects.**", ""]
        + _boundaries_block(stamp),
