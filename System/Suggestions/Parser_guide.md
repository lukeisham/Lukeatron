...
| | | | *↑ **Verification engine BUILT 2026-09-13** as an agent-run Skillbank skill, not the browser cartridge: `!FactCheck` (`System/Skillbank/Teaching/!FactCheck/skill.md`, v1.0.0) runs `FactCheckingParser.spec.md`'s Stage 1–4 pipeline directly with the session's own WebSearch/WebFetch, batched in ≤250-word passes, reusing the spec's five-category taxonomy and 17-row whitelist verbatim. Sandbox-tested live (`System/Sandbox/factcheck-skill-test.md`). Per Luke's explicit architecture decision, the browser widget/cartridge remains unbuilt and is TE-08's tracked next action — this row's Tier/cap/logic describe the eventual widget, already matched by the skill's own cap and batching.* | |
...
| Fact-checking | The `Claim \| Chunk \| Source \| Category` table, spec `FactCheckingParser.spec.md` §6 — implemented by `!FactCheck` (2026-09-13). |
...
- Fact-checking: approved source list resolved 2026-09-13 — the spec's own 17-row whitelist (§4)
  serves both the `!FactCheck` skill (built) and the still-unbuilt browser cartridge.
