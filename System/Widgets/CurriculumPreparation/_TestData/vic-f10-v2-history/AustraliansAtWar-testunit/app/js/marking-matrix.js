...
export class MarkingMatrix {
  ...
  addCriterion(tier, criterionText, assessmentIds) {
    ...
    const warning = hasScored
      ? `Adding a criterion adds an unmarked row to every student, lowering their totals until marked.`
      : null;
  ...
  editCriterion(criterionId, updates) {
    ...
    if (updates.tier !== undefined && updates.tier !== criterion.tier) {
      ...
      reallocationWarning = `Changing this criterion's tier may re-allocate marks for every student in both tiers.`;
    }
  ...
  renderStudentDataPrint(studentId) {
    ...
    const renderFn = (pageData, svgElement) => {
      ...
      studentName.setAttribute('font-size', '14');
      ...
      for (const tier of tiers) {
        ...
        subtotalText.setAttribute('font-size', '10');
  ...
  renderSetupPrint() {
    ...
    const renderFn = (pageData, svgElement) => {
      ...
      totalLine.setAttribute('font-size', '10');
