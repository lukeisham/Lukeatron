...
test('D1: assessment material/studentTask survive a save -> load round-trip through local-store', async () => {
  ...
  doc.render(svg);
  assert.ok(
    svg.children.some(el => el.textContent && el.textContent.includes('Read the source excerpt')),
    'renderer should draw the round-tripped material text'
  );
  assert.ok(
    svg.children.some(el => el.textContent && el.textContent.includes('Answer the three questions')),
