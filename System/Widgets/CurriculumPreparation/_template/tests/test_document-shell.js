...
import { DocumentShell, TIERS, pxToUserUnits, FONT_SIZE_UU } from "../app/js/document-shell.js";
...
// ===== pxToUserUnits (px -> SVG user-unit / mm conversion) =====
// The document surface is an SVG with viewBox="0 0 210 297" and
// width="210mm", so 1 SVG user unit = 1mm. A font-size set as a bare
// SVG attribute is unitless and resolves as user units, so a CSS px value
// must be converted through pxToUserUnits() before use in setAttribute().
test("pxToUserUnits: converts known px sizes to the documented mm values", () => {
  assert.equal(pxToUserUnits(9), 2.38, "9px -> 2.38 user units");
  assert.equal(pxToUserUnits(10), 2.65, "10px -> 2.65 user units");
  assert.equal(pxToUserUnits(11), 2.91, "11px -> 2.91 user units");
  assert.equal(pxToUserUnits(13), 3.44, "13px -> 3.44 user units");
  assert.equal(pxToUserUnits(14), 3.7, "14px -> 3.70 user units");
});

test("pxToUserUnits: is the inverse of the 96dpi px-per-mm ratio", () => {
  const px = 24;
  const uu = pxToUserUnits(px);
  // uu * (96/25.4) should round-trip back to roughly the original px value
  assert.ok(Math.abs(uu * (96 / 25.4) - px) < 0.05, "round-trips within rounding tolerance");
});

test("pxToUserUnits: scales monotonically and never returns a larger number than its input at typical font sizes", () => {
  // Regression guard for the exact bug class this fixes: a converted value
  // must always be smaller than the raw px value it replaces (since 96dpi
  // px-per-mm > 1), otherwise the "fix" reproduces the oversized-text bug.
  for (const px of [6, 8, 9, 10, 11, 12, 13, 14, 18]) {
    assert.ok(pxToUserUnits(px) < px, `${px}px converts to a smaller user-unit value`);
  }
});

test("FONT_SIZE_UU: pre-converted constants match pxToUserUnits()", () => {
  for (const px of Object.keys(FONT_SIZE_UU)) {
    assert.equal(FONT_SIZE_UU[px], pxToUserUnits(Number(px)), `FONT_SIZE_UU[${px}] matches pxToUserUnits(${px})`);
  }
});
