...
def assemble(cartridge_dir: Path, output: Path | None) -> Path:
    ...
    out_path = output if output else cartridge_dir / f"{cartridge_id}_generator.html"
    ...
    present_mode_js = _read(SHELL_DIR / "present-mode.js")
    ...
    # Task B: validation branches on generator.mode, but stays exactly as
    # strict as Parser's single-mode check either way — a cartridge is
    # rejected at build time for a missing export, never left to fail at
    # runtime in the browser (PY-6).
    if generator_mode == "present":
        generator_cfg = config.get("generator") or {}
        required_present_keys = set(REQUIRED_PRESENT_ENGINE_KEYS)
        if generator_cfg.get("answer"):
            required_present_keys.add("checkAnswer")
        ...
        # both modes, §3b), but its export set is not checked in this mode.
    else:
    ...
    if generator_mode == "present" and not content:
        raise CartridgeError(
            "generator.mode is 'present' but files.content compiled to an empty pool — "
            "present mode requires a non-empty baked-in item pool (files.content, a JSON "
    ...
    # Fallback favicon glyph tracks __BADGE_GLYPH__ (first letter of
    # cartridge.id) rather than a fixed letter — a hardcoded "P" here would
    # be exactly the Parser-idiom bleed-through the Generator fork exists to
    # remove (a cartridge that omits `icon` would ship with the wrong
    # letter's badge).
    badge_glyph = (config["cartridge"]["id"][:1] or "G").upper()
    default_icon = (
        ...
        f"font-size=\"16\" font-weight=\"bold\" fill=\"%23faf9f5\">{badge_glyph}</text></svg>"
    )
    ...
    substitutions = {
        ...
        "__UI_JS__": ui_js,
        "__PRESENT_MODE_JS__": present_mode_js,
        "__API_JS__": api_js,
    }
...
def validate_manifest(config: dict) -> None:
    ...
    # Present-mode-only fields (task D): categories, if declared, must each
    # carry id + label so the shell's category picker never renders a blank
    # option.
    generator_cfg = config.get("generator") or {}
    if generator_cfg.get("mode") == "present":
        categories = generator_cfg.get("categories")
        if categories is not None:
            if not isinstance(categories, list) or len(categories) == 0:
                raise CartridgeError("generator.categories, if present, must be a non-empty list")
            for i, cat in enumerate(categories):
                for field in ("id", "label"):
                    if not isinstance(cat, dict) or field not in cat:
                        raise CartridgeError(f"generator.categories[{i}] missing field '{field}'")
...
def build_config_js(config: dict) -> str:
    ...
    obj = {
        ...
        # Task B — the mode switch, and (present mode only) the feature
        # flags present-mode.js reads to decide which controls to show
        # (task D: category picker, clue button, answer bar, explainer
        # panel). "categories" is omitted entirely when the cartridge
        # doesn't declare any, matching the sweeps/miniwiki "absent means
        # off, never an empty-but-present list" convention already used
        # above.
...
def build_generator_config(config: dict) -> dict:
    """CONFIG.generator — the mode switch (task B) plus present-mode's
    feature flags (task D). validate_manifest() already confirmed
    generator.mode is one of GENERATOR_MODES and that any declared
    categories carry id+label, so this function only shapes the object —
    it does not re-validate."""
    generator_cfg = config.get("generator") or {}
    mode = generator_cfg["mode"]
    obj: dict = {"mode": mode}
    if mode != "present":
        return obj
    obj["clue"] = bool(generator_cfg.get("clue", False))
    obj["answer"] = bool(generator_cfg.get("answer", False))
    obj["explainer"] = bool(generator_cfg.get("explainer", False))
    categories = generator_cfg.get("categories")
    if categories:
        obj["categories"] = [{"id": cat["id"], "label": cat["label"]} for cat in categories]
    return obj
