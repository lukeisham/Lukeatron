...
MIN_PRESENT_ENGINE = (
    "var ENGINE = (function(){\n"
    "function getPool(){return [{id:'a',text:'Alpha'},{id:'b',text:'Beta'}];}\n"
    "function render(item){return {html:'<p>'+item.text+'</p>',canonicalText:item.text};}\n"
    "return {getPool:getPool,render:render};\n"
    "})();\n"
)
...
class TestAssemble(unittest.TestCase):
    ...
    # ---- generator.mode (task B) ----

    def test_missing_generator_mode_is_rejected_and_named(self) -> None:
        bad_yaml = MIN_CONFIG_YAML.replace("generator:\n  mode: analyse\n", "")
        with tempfile.TemporaryDirectory() as tmp:
            cartridge = make_cartridge(Path(tmp) / "Test", config_yaml=bad_yaml)
            with self.assertRaises(assemble.CartridgeError) as ctx:
                assemble.assemble(cartridge, cartridge / "out.html")
    ...
    def test_invalid_generator_mode_value_is_rejected_and_named(self) -> None:
        bad_yaml = MIN_CONFIG_YAML.replace("mode: analyse", "mode: something-else")
        with tempfile.TemporaryDirectory() as tmp:
            cartridge = make_cartridge(Path(tmp) / "Test", config_yaml=bad_yaml)
            with self.assertRaises(assemble.CartridgeError) as ctx:
                assemble.assemble(cartridge, cartridge / "out.html")
    ...
    def test_present_mode_cartridge_assembles_with_present_engine_contract(self) -> None:
        present_yaml = MIN_CONFIG_YAML.replace("mode: analyse", "mode: present") + (
            "files:\n  engine: \"engine.js\"\n  explainer: \"explainer.js\"\n"
            "  content: \"pool.json\"\n"
        )
        with tempfile.TemporaryDirectory() as tmp:
            cartridge = make_cartridge(Path(tmp) / "Test", config_yaml=present_yaml, engine=MIN_PRESENT_ENGINE)
            (cartridge / "build" / "pool.json").write_text(
                '{"a": {"id": "a", "text": "Alpha"}, "b": {"id": "b", "text": "Beta"}}',
                encoding="utf-8",
            )
            out = assemble.assemble(cartridge, cartridge / "out.html")
            html = out.read_text(encoding="utf-8")
            self.assertIn('"mode": "present"', html)
            self.assertEqual(re.findall(r"__[A-Z_]+__", html), [])

    def test_present_mode_missing_render_export_is_rejected_and_named(self) -> None:
        present_yaml = MIN_CONFIG_YAML.replace("mode: analyse", "mode: present") + (
            "files:\n  engine: \"engine.js\"\n  explainer: \"explainer.js\"\n"
            "  content: \"pool.json\"\n"
        )
        bad_engine = MIN_PRESENT_ENGINE.replace("return {getPool:getPool,render:render};", "return {getPool:getPool};")
        with tempfile.TemporaryDirectory() as tmp:
            cartridge = make_cartridge(Path(tmp) / "Test", config_yaml=present_yaml, engine=bad_engine)
            (cartridge / "build" / "pool.json").write_text('{"a": {"id": "a"}}', encoding="utf-8")
            with self.assertRaises(assemble.CartridgeError) as ctx:
                assemble.assemble(cartridge, cartridge / "out.html")
            self.assertIn("render", str(ctx.exception))

    def test_present_mode_without_pool_is_rejected(self) -> None:
        present_yaml = MIN_CONFIG_YAML.replace("mode: analyse", "mode: present")
        with tempfile.TemporaryDirectory() as tmp:
            cartridge = make_cartridge(Path(tmp) / "Test", config_yaml=present_yaml, engine=MIN_PRESENT_ENGINE)
            with self.assertRaises(assemble.CartridgeError) as ctx:
