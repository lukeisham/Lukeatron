...
function makeSandbox({ levels = ["sentential", "clausal", "phrasal", "lexical"], spellingAvailable = false, sweeps = undefined } = {}) {
  ...
  const ENGINE = {
    ...
    parse(text, selection) { parseCalls.push({ text, selection }); return fixedParseResult(); },
  };
  const EXPLAINER = {
    posShort() { return "N"; },
  ...
  sandbox.window = sandbox; // ui.js does `window.GeneratorParseResult = ...`
...
test("display harness: parseNow() renders a fixed ParseResult into #stage with escaped word tokens", () => {
  ...
  // window.GeneratorParseResult is set by the real parseNow() -- assert the actual
  // state change (TEST-6), not merely that nothing threw.
  assert.equal(sandbox.window.GeneratorParseResult.result.meta.wordCount, 2);
});
